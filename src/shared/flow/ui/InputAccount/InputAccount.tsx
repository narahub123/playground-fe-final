...
const InputAccount = ({
  ...
  useEffect(() => {
    // inputValue의 field 구별하기
    const type = determineInputValueType(inputValue);
    ...
    const checkUserExistence = async () => {
      ...
      if (type === "email") {
        const response = await checkEmailDuplicateInSignupAPI(inputValue);
        ...
        const response = await checkUserIdDuplicateInSignupAPI(inputValue);
  ...
  // 언어 설정
  const { label } = useLanguageContent(["components", "InputAccount"]);

  return (
    <Input
      className={className}
      disabled={disabled}
      field="email, phone, userId"
      label={label}
